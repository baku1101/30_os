...
unsigned int memman_alloc_4k(struct MEMMAN *man, unsigned int size);  // 0x1000(4KB)単位にサイズを変更してallocに投げる
...
struct SHEET {
	unsigned char *buf;
	int bxsize, bysize, vx0, vy0, col_inv, height, flags; // color_invisibleの略
	struct SHTCTL *ctl;
};
struct SHTCTL {
...
void sheet_updown(struct SHEET *sht, int height); // シートの高さを設定する関数
void sheet_refresh(struct SHEET *sht, int bx0, int by0, int bx1, int by1);
void sheet_slide(struct SHEET *sht, int vx0, int vy0);
void sheet_free(struct SHEET *sht);

/* timer.c */
#define MAX_TIMER		500 //タイマーの数の最大数
struct TIMER {
